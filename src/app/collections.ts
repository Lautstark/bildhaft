...
import SayFoot from '../ui/SayFoot.svelte';
import SayBody from '../ui/SayBody.svelte';
...
export function scheduleSearch(): void {
  ...
  searchTimer = window.setTimeout(() => {
    void searchSentences(s.query).then((hits) => { s.results = hits; });
  }, 200);
}
...
export async function syncProvider(): Promise<void> {
  if (s.previousProvider === providerId()) return;
  s.previousProvider = providerId();
  await resolveOpen();
}
 ...
 */
export async function resolveOpen(): Promise<void> {
  const id = s.activeId;
  if (!id) return;
  ...
  try {
    ...
    if (s.activeId === id) s.sentences = merged;
...
export async function handleNewCollection(): Promise<void> {
  ...
  /* Straight into the name, selected: the first keystroke replaces the date
     ...
     delete, which is the difference between a suggestion and a default. The
     field takes the caret from its own effect, which runs after the paint that
     puts the new name in it. */
  askFor('collection-name');
}
...
export async function confirmDeleteCollection(): Promise<void> {
  const collection = activeCollection();
  if (!collection) return;
  ...
  await refreshCollections();
  if (remaining[0]) { s.sentences = []; setActive(remaining[0].id); }
 ...
 */
export function openSourceSheet(): void {
  const collection = activeCollection();
...
export async function confirmClearAll(): Promise<void> {
