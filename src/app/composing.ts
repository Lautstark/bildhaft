...
import { holdsWords, provider, providerId } from './state.ts';
import { LOOKUP_MS, STORE_MS, sayWhy } from './waiting.ts';
import type { Ctx } from './context.ts';

export interface Composing {
  scheduleReuseLookup(): void;
  handleSubmit(): Promise<void>;
  handleReuse(): Promise<void>;
}
...
export function composing(ctx: Ctx): Composing {
  const { s } = ctx;
  ...
  function scheduleReuseLookup(): void {
    ...
    reuseTimer = window.setTimeout(async () => {
      ...
      if (normalizeInput(s.draft) === normalized) { s.reuse = hit; ctx.render(); }
    }, 320);
  }
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      ...
      ctx.render();
      const words = holdsWords(s);
      const options = {
        provider: provider(s),
        stopwords: new Set(s.settings.stopwords[LANG]),
        overrides: await overrideMap(providerId(s)),
      };
      ...
      if (firstError !== null) ctx.notify(sayWhy(firstError));
      ctx.render();
    }
  }
  ...
  return { scheduleReuseLookup, handleSubmit, handleReuse };
}
