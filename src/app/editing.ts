...
import { openSlotPicker } from '../ui/slotPicker.svelte.ts';
import { resetSymbolResolution } from '../ui/symbols.ts';
import { kind, providerId, s } from './state.svelte.ts';
 ...
 */
export function handleRename(sentenceId: string, title: string): Promise<void> {
  return queued(async () => {
...
export function openPicker(sentenceId: string, slotId: string): void {
...
export async function handleAddSlot(sentenceId: string): Promise<void> {
  const slot: Slot = {
    id: newId(),
    sourceToken: '',
    concept: '',
...
export async function handleReorder(sentenceId: string, from: number, to: number): Promise<void> {
  await mutateSlots(sentenceId, (slots) => {
    const next = [...slots];
...
export async function confirmDeleteSentence(sentence: Sentence): Promise<void> {
...
export async function noteUnreadable(id: string): Promise<void> {
