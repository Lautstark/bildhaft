<script lang="ts">
  ...
  let { slot, settings, provider, fill = false, background, block }: {
    slot: Slot;
...
<div class="ps-card{settings.labelPosition === 'above' ? ' ps-card--label-above' : ''}{fill ? ' ps-card--fill' : ''}{framed ? ' ps-card--framed' : ''}" style:--card-bg={background}>{#if block}{#each block.joints as joint, at (at)}<div class="ps-joint" style={asStyle(joint)}></div>{/each}<div class="ps-block" style:--zone={block.fill} style:inset={block.inset} style:border-radius={block.borderRadius} style:border-color={block.frame ?? 'transparent'} style:border-width={block.borderWidth} style:clip-path={block.clipPath}></div>{/if}{#if framed}<div class="ps-card__frame">{@render contents()}</div>{:else}{@render contents()}{/if}{#if block?.shield}<!-- The name, as a shield on the block's top left edge, on the group's first field. --><div class="ps-shield" style:--shield={block.shield.colour}>{block.shield.text}</div>{/if}</div>
