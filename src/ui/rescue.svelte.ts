...
export class Rescuing {
  ...
  constructor(readonly dump: Dump, readonly page: Page) {}

  get held(): number { return countRecords(this.dump); }

  async save(): Promise<void> {
    try {
      downloadJson(await asFile(this.dump, t('ui.rescue_notice')), 'rettung');
      this.said = t('ui.rescue_saved');
      // Only now. The file is the whole of what makes the button beside it
      // survivable.
      this.saved = true;
    } catch (failure) {
      this.said = t('ui.rescue_failed', { error: reason(failure) });
    }
  }

  async discard(): Promise<void> {
    this.going = true;
    this.said = t('ui.rescue_discarding');
    try {
...
async function show(page: Page): Promise<void> {
  ...
  const sheet = openSheet({
    ...
    onClose: () => { if (!state.going) page.report(t('ui.rescue_stopped')); },
  });
