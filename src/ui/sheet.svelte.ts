 ...
 * Lautstark programme draws it, with this app's words supplied by dialog.ts
 * beside this file. What this adds is that the body and, where one is needed,
 * the foot are Svelte components mounted straight into the frame's own
 * containers, sharing one state object the opener made. No wrapper element
 * between the frame and the content, because components.css styles the frame's
 * children directly and a box in between would take those rules away — which
 * the visual baselines would see at a tolerance of zero.
 *
 * wochenwerk's pilot wrote this first (`src/kalender/sheet.svelte.ts`); adr/0003
 * says why it is copied rather than shared, and what would have to be true for
 * it to move into a package.
 */
