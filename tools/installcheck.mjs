...
// shared packages are git dependencies, nothing in the toolchain compares the
// installed copy with the pin, and the way it surfaces is as a test failing
// somewhere far away.
//
// Ported from vorlaut, which has had it since 2026-08-24, on the day mitreden
// produced the failure it exists to prevent. The design package was released at
// 1.15.0 with .btn.sm defined for the first time and the pins were updated, but
// node_modules in the main checkouts still held 1.14.1. What went red over
// there was a test about edge alignment, off by two pixels, in code that was
// correct - because .btn.sm did not exist in the installed copy and the button
// kept .btn's larger padding.
//
// This repository is exposed to exactly that. Its suite measures things the
// shared stylesheet draws - the Sammlung rows, the title field, the small
// button - so a design package one release behind produces numbers that are
// wrong by a few pixels and a story about layout that is entirely plausible.
//
// That is worse than an error, and it is why this is a preflight rather than a
// test: a failure among other failures is exactly the noise it removes.
//
...
//
// Kept byte-identical to vorlaut's and bildhaft's below the header, on purpose.
// The logic is generic - it compares metadata npm wrote, and names no package -
// so three copies of one rule is the cheapest arrangement that does not drift:
// there is nothing here to keep in step except the comment above.
